...
// filter id list of product, default product(0) used if not specified
...
func (r PlanPlanListGetRequest) Tqype(tqype []int32) PlanPlanListGetRequest {
	r.tqype = &tqype
	return r
}
...
func (a *PlanService) PlanListGetExecute(r PlanPlanListGetRequest) (*MerchantPlanListGet200Response, *http.Response, error) {
	...
	if r.tqype != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "tqype", s.Index(i).Interface(), "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "tqype", t, "multi")
		}
	}
	if r.status != nil {
		t := *r.status
